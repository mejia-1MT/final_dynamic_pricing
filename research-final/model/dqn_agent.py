...
# Define the agent
class DQNAgent:
    def __init__(self, input_size, output_size, initial_price, memory_capacity=10000, batch_size=32, learning_rate=0.001, gamma=0.88, epsilon=1.0, epsilon_decay=0.999, epsilon_min=0.01):
    ...
    def choose_action(self, aggregated_state):  
        if random.uniform(0, 1) < self.epsilon:
            action = random.randint(0, self.num_actions - 1)  # Choose a random action
            # print(f"EXPLOREEEEEE action: {action}")
        else:
            # print(aggregated_state)
            q_values = self.model(torch.Tensor(aggregated_state))
            # print(f"q_values {q_values}")
            action = torch.argmax(q_values).item()
            # print(f"\t\texploit action: {action}")
        
        self.epsilon *= self.epsilon_decay
        self.epsilon = max(self.epsilon, self.epsilon_min)
        price = self.pricing(action + 5)
        return price
